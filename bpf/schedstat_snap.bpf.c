...
//
// Idea (see plan): every thread has a monotonically increasing CPU odometer
// (run = se.sum_exec_runtime, wait = sched_info.run_delay). We keep, per thread,
// the odometer value we last counted (the "baseline book", map `last`). Twice
// we look at a thread:
//   1) the 5s sweep (iter/task) reads every LIVE thread, adds (now - last) to
...
// --- bucket ids for the fixed per-slice scoreboard (agg_slice) ---
#define BUCKET_OTHER    0   // anything not under the 3 tracked slices
...
// 0 means "not provided" and will simply never match.
...
// Per-slice scoreboard (4 fixed buckets). Per-CPU so updates are lock-free;
...
static __always_inline void account(struct task_struct *task, int is_exit)
{
    ...
    if (prev && run >= prev->run) {
        ...
        // brand-new this round, or tid reuse / non-monotonic -> count from 0
        drun  = run;
        dwait = wait;
    }
...
// 5-second sweep: userspace triggers this iterator, kernel walks every task.
SEC("iter/task")
...
// Catch threads that die between sweeps.
